...
from utils.train_utils import SalaryPredictor, BatchLoader, evaluate
from utils.logs import get_logger

TARGET_COLUMN = "Log1pSalary"

def train(epochs: int, train_batches: str, validation_batches: str, features_dim: str, model_dir: str, device: str) -> None:
    ...
    train_loader = BatchLoader(train_batches)
    val_loader = BatchLoader(validation_batches)
    
    logger.info("Extract length of vocabulary and number of categorical features from features_dim.csv") 
    features_dim = pd.read_csv(features_dim)
